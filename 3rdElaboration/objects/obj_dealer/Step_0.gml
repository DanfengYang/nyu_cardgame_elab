...
switch (global.phase){
		...
		if (wait_timer > 100){
			...
			if (play_computer.type == global.paper){
				if (play_player.type == global.rock){
					...
					global.player_score= global.player_score +2;
					global.girl = 3;
					audio_play_sound(snd_hug,0,0);
	...
	case global.phase_cleanup:
		...
		if (move_timer%4 == 0){
			...
			if (index >= 0){
				...
				card.face_up = false;
				card.target_x = 40;
				card.target_y = 200 - 2*ds_list_size(deck);
	...
	case global.phase_reshuffle:
		...
		if (move_timer%4 == 0){
			...
			card.target_y = 200- 2*current_card;
			card.y = 200 - 40;
